...
public class GameResource {
  ...
  public ResponseEntity<Page<GameResponse>> getGames(
    ...
    Page<GameResponse> result =
        gameService.searchGames(q, genre, platform, year, pageable).map(GameConverter::toResponse);
  ...
  public ResponseEntity<GameResponse> updateGame(
      @PathVariable UUID id, @RequestBody @Valid GameUpdateRequest request) {
  ...
  @GetMapping("/recent")
  public ResponseEntity<List<GameResponse>> getRecentGames() {
    return ResponseEntity.ok(
        gameService.getRecentGames(10).stream().map(GameConverter::toResponse).toList());
  }

  @GetMapping("/popular")
  public ResponseEntity<List<GameResponse>> getPopularGames() {
    return ResponseEntity.ok(
        gameService.getPopularGames(10).stream().map(GameConverter::toResponse).toList());
  }
}
